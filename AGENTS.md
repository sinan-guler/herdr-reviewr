...
- **No content writes**: reviewr never mutates file content, the worktree, or branches, and never commits. It writes the index only under the reviewer's own `stage`/`unstage` keypress (the review mark), and only whole paths — never content, never a hunk. Its only other git writes are private refs under `refs/worktree/reviewr/`: the turn baseline and the base pick. Every read path stays write-free, and `tests/git_repo.rs` holds both halves: the read functions mutate nothing, and `stage_paths` moves the index and nothing else.
...
- `src/git.rs` — every git subprocess. `changed_files` (scope changesets; `Unstaged` is a base-less `git diff`, i.e. index vs worktree), `all_files` (tracked + untracked + ignored via `ls-files` — never use `git status --ignored`, it walks inside ignored trees and costs seconds), `snapshot_worktree` (temp-index `add -A` + `write-tree` for turn baselines), baseline refs, and the review mark: `stage_paths`/`unstage_paths` (the only index writes, always `--literal-pathspecs` — a pathspec is a glob by default and `lit[ab].txt` would otherwise reach a sibling), `mark_staged` (whole changeset, worker-side) and `staged_state` (one path, read back after a write).
...
- `src/model.rs` — `Scope` (five), `Staged` (the review mark: `No`/`Partial`/`Yes`, where `Partial` means reviewed-then-changed-again), `CommentStore` (in-memory), comment anchoring (`diff_anchored` distinguishes diff comments from All-files content comments — each renders only in its own view).
...
The user tests builds in real herdr panes. The panes run the GitHub-installed plugin's binary at `~/.config/herdr/plugins/github/sinan-guler.reviewr-<hash>/bin/herdr-reviewr`, NOT anything in this worktree. Full procedure: `docs/qa-install.md`. Short form:
